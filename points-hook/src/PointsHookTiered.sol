...
contract PointsHook is BaseHook, ERC1155 {
    ...
    mapping(uint256 => uint256) public tierPoints; // Tier to rewards multiplier

    constructor(
        IPoolManager _manager
    ) BaseHook(_manager) {
        tierPoints[0] = 500; // 5% bps rewards for swapping ETH to Token
        tierPoints[1] = 1000; // 10% bps rewards for swapping ETH to Token
        tierPoints[2] = 2000; // 20% bps rewards for swapping ETH to Token
        tierPoints[3] = 3000; // 30% bps rewards for swapping ETH to Token
        tierPoints[4] = 4000; // 40% bps rewards for swapping ETH to Token
    }
	...
	function _afterSwap(
       ...
       // If this is not an ETH-TOKEN pool with this hook attached, ignore
        ...
        uint256 tier = 0; 
        if (ethSpendAmount >= 100 ether) {
            tier = 4;
        } else if (ethSpendAmount >= 50 ether) {
            tier = 3;
        } else if (ethSpendAmount >= 25 ether) {
            tier = 2;
        } else if (ethSpendAmount >= 10 ether) {
            tier = 1;
        }

        uint256 pointsForSwap = ethSpendAmount * tierPoints[tier] / 10_000;
